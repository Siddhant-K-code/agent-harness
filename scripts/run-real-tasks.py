...
def main():
    ...
    for case in manifest["cases"]:
        ...
        if hashlib.sha256(verifier_bytes).hexdigest() != case["verifier_sha256"]:
        ...
        cases.append((case, spec, verifier_bytes))
    ...
    save(path, journal)
    for case, spec, verifier_bytes in cases:
        # Persist the exact bytes validated during admission, before any dispatch.
        verifier = output / (case["id"] + ".verify.sh")
        verifier.write_bytes(verifier_bytes); verifier.chmod(0o600)
        spec["verifier"] = str(verifier)
        task = output / (case["id"] + ".task.json"); save(task, spec)
    for case, spec, _ in cases:
        task = output / (case["id"] + ".task.json")
