...
Design and implementation status, September 5, 2026. Region: `us-east-1`. Docker remains the default executor. The [full AWS adapter](aws-harness.md) adds model-loop integration, bounded artifact transfer and whole-runtime reconciliation; its implementation supersedes the earlier proposed contract below. The separately implemented AgentCore probe [passed twelve real checks, native tracing, and teardown](evidence/2026-09-05/aws-isolation/README.md). Its scoped bootstrap remains deployed. The first $50 experiment allocation was authorized; the remaining allocations below are proposals.
...
The implemented probe uses a minimal private ECR image, scoped IAM roles, short-lived logs, and the AWS SDK for Go v2 command stream. Its Go controller runs on GitHub Actions through OIDC. The original probe transfers known fixtures in bounded command payloads. The full adapter transfers checksummed workspace archives through authenticated invocations; S3 storage remains optional future work. The checked-in infrastructure and workflow implement runtime/image/log teardown. The client now mandates a seccomp allowlist for every command. The verifier denies filesystem writes globally in a fresh session, and actual command journals export natively to AgentTrace. The following steps describe the broader backend acceptance work.
