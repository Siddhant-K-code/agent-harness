...
Design proposal, September 5, 2026. The [local runtime](architecture.md) is implemented; the integrations below are not. This plan follows inspection of the linked projects' code.
...
1. Add `internal/trace/agenttrace` and a pinned Python bridge in `integrations/agenttrace`. Python is needed for export, not for running the agent.
