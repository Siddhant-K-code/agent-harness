...
The [AWS AgentCore capability probe](infra/aws/agentcore/README.md) passed nine real cloud checks in `us-east-1`, including the existing GPT-5.4 patch, fresh-session verification, streaming, timeout, controller disconnect, and confirmed runtime teardown. It made no new model calls. See the [live report and cleanup audit](docs/evidence/2026-09-05/aws-agentcore/README.md). The probe remains separate from the production model runner because network restrictions, a read-only verifier workspace, and session-absence inspection are not enforced. GitHub Actions uses OIDC and two repository variables; no Actions secrets are required.
